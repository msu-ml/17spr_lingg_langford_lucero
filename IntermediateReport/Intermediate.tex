...
\fancyhf{}
\lhead{
	CSE 847: Machine Learning---Intermediate Report \\
...
\begin{document}
	...
	\begin{multicols}{2}
		\subsection{Linear Regression}
			One model we are using to characterize the housing prices is a basic ridge regression using the closed form solution. The optimal regularization lambda is determined using cross training. Table \ref{table:linr_cross_train} shows the results of the cross training, 1e-06 is producing the best cross training results. The model is currently being trained using the first 50\% of the training set and tested against the second 50\%.
		\par
		{\centering
        	\captionsetup{type=table}
			\begin{tabular}{r|c}
				& \small{Cross Test MSE} \\
				\hline
				\small{1e-07} & \small{2.109e+10} \\
				\hline
				\small{1.5e-07} & \small{2.046e+10} \\
				\hline
				\small{1e-06} & \small{1.996e+10} \\
				\hline
				\small{1.5e-06} & \small{2.003e+10} \\
				\hline
				\small{1e-05} & \small{2.049e+10} \\
				\hline
			\end{tabular}
			\captionof{table}{Linear regression lambda cross training.}
			\label{table:linr_cross_train}        
		\setlength{\parindent}{15pt} }
		\par
			In Figure \ref{fig:linr_predictions} we show how our simple linear regression model performs when the data is sorted by price. The model performs best at lower prices, where the training data is the most dense. For higher value homes, the model estimates lower values. Additional work will be performed to improve this fit. Current plans are to train against the log of the home prices or see if splitting homes into a small number of categories and training a model for each category will be effective. Additional plans are see how an ensemble approach works, this will at least be an ensemble of linear regression models and may include combining other modelling techniques.
		\par
		\begin{center}
            \captionsetup{type=figure}
			\includegraphics[scale=0.35]{Images/linearRegression} \\
			\captionof{figure}{Linear regression predictions.}
		...
		\par
		For our Decision Tree approach, we used a Classification Decision Tree and a Regression Decision Tree. For the Classification Tree, since the data was not already classified, it is split into \(k\) equally sized classes where each class represents a range of prices. The average value for every feature column is also calculated and used to subsitute for any missing data points in the observations. A classification model was built for the KC Data Set, Refined Data Set, and Nashville Dataset. For each data set, initially a classification tree model was built using crossfold validation with 10 folds using 90\% of the data for training. This model used the default parameters for n-1 max number of splits, 1 for the minimum leaf size, and 10 for the minimum parent size. 
		\par		
Another classification model was built using 10 fold cross validation and iterating over possible parameter values to choose the parameters that provide the best training MSE, again using 90\% of the data for training. The results comparing these two models and their training and testing MSEs for the Nashville data set is shown in Table \ref{table:ctree_performance} below. The MSE indicates the averae squared distance between the predicted class and the actual class, where the classes are represented by a range of integers from 1 to \(k\). 
		\par
		\captionsetup{type=table}
			\begin{tabular}{r|c}
				& \small{Mean Squared Error} \\
			...
			\setlength{\parindent}{15pt}
		\par
		For the Classification Tree, the optimized parameters result in much higher testing error, but also a much better testing error. This seems to indicate that default parameters with crossfold validation result in slight overfitting compared to the optimized parameters.
		\par
		Regression Tree models were generated following a similar pattern, creating one 10 fold cross validation Regression Tree model for each data set and creating a Regression Tree model by varying the tree parameters to find the best MSE training error. Table \ref{table:rtree_performance} below shows a table of the MSE results for the Regression Tree approach on the Nashville data set. 
		\par
		\captionsetup{type=table}
			\begin{tabular}{r|c}
				& \small{Mean Squared Error} \\
			...
			\setlength{\parindent}{15pt}
		\par
		The Regression models perform similarly, with slightly better results using the optimized parameters for the regression tree compared to the default parameters. Our goal for the Decision Tree approach is to take the best approach and vary the feature selection to create a Random Tree Forest. Then use proximity trees to better estimate any missing values from the data sets. 
