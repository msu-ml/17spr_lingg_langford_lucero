...
load ( strcat('../../../../Data/Predictions/',fileName,'_log_reg_predictions.mat') );
...
y_train = tblArray(1:size(tblArray,1)/2,1) - predictionArray(1,1:size(predictionArray,2)/2)';
...
y_pred = predictionArray(:,size(predictionArray,2)/2+1:size(predictionArray,2))';
...
for lambdaIndex = 1:9
  ...
  crossResults( lambdaIndex ) = CrossValidation(testLambdas(lambdaIndex), y_train, x_train);
...
W_ML = ( bestLambda*eye(size(x_train,2)) + x_train'*x_train ) \ x_train' * y_train; 

    figure;
    plot ( W_ML, 'r' );
    legend('BestWeights');

disp(TestError(W_ML,y_pred,y_test,x_test,true,true,fileName));
...
function MSE = CrossValidation(LambdaValue, Truth, Data)
  ...
  for i = 0:3
    ...
    runningMSE = runningMSE + CrossError(W_ML, testTruth, crossTestData, false, false);
  end
  ...
  % Truth is the actual results corrisponding with the input Data,
  % ModelW and Bias are our model to test.
function MSE = TestError(ModelW, Prediction, Truth, Data, Graph, DumpData, FileName)
  MSE = 0;

    % Calculate the MSE of each data sample.
  result = zeros( 1, length(Truth) );
  for i = 1:length(Truth)
    result( i ) = ModelW' * Data( i, : )' + Prediction( i );
